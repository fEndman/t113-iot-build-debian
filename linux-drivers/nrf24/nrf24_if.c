			   ...
			   loff_t *f_pos)
{
	...
	while (left > 0) {
		data.size = p->cfg.plw != 0 ? p->cfg.plw : min_t(size_t, left, PLOAD_MAX);

		memset(data.pload, 0, PLOAD_MAX);
		if (copy_from_user(data.pload, buf + copied, min_t(size_t, left, data.size)))
			goto exit_lock;
...
static int nrf24_gpio_setup(struct nrf24_device *device)
{
	...
	device->ce = gpiod_get(&device->spi->dev, "ce", GPIOD_OUT_LOW);
...
static int nrf24_hal_init(struct nrf24_device *device)
{
    ...
    /* When the system is not powered down and restarted, sending this command 
       again will cause the chip function to be turned off. Therefore it is 
       necessary to determine if it is the first time to start. */
