...
static ssize_t available_channel_show(struct device *dev,
			      struct device_attribute *attr,
			      char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "0 - 127\n");
}
			       ...
			       size_t count)
{
	...
	if (tmp != ret) {
		ret = nrf24_set_rf_channel(device->spi, (tmp & 0x7f));
		if (ret < 0)
			return ret;
		dev_dbg(dev, "%s: new channel = %d\n", __func__, tmp);
	}
	return count;
}
...
static DEVICE_ATTR_RO(available_channel);
...
struct attribute *nrf24_attrs[] = {
	...
	&dev_attr_available_channel.attr,
	NULL,
};
